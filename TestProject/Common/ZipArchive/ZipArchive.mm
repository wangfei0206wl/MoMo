...
-(NSData *) UnzipFileToData
{
    ...
    unsigned char *uncompressedBuffer = (unsigned char*)malloc((fileInfo.uncompressed_size + 1) * sizeof(unsigned char*));
    int read = unzReadCurrentFile(_unzFile,uncompressedBuffer,fileInfo.uncompressed_size);
    if(read < 0)
    {
        [self OutputErrorMessage:@"Failed to reading zip file"];
    ...
    uncompressedBuffer[fileInfo.uncompressed_size] = 0;
    NSData *unZippedData = [[NSData alloc] initWithBytes:uncompressedBuffer length:fileInfo.uncompressed_size];
    free(uncompressedBuffer);
    unzCloseCurrentFile( _unzFile );
